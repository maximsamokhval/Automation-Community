...
--  3. Если таблица в перечне обязаельных таблиц, тогда реиндексация
...
DECLARE @partition_descr nvarchar(128);
-- описание партиции, которую нужно обработать. может быть пустым
...
-- Цикл по секциям
FETCH NEXT FROM partitions INTO @object_id, @index_id, @partition_number,
...
WHILE @@FETCH_STATUS = 0
    ...
    IF @partition_number > 1
            SET @partition_descr = N' PARTITION=' + CAST(@partition_number AS nvarchar(10));
 ELSE 
       SET @partition_descr = N''


    -- Если фрагментация менее или равна 30% тогда дефрагментация, иначе реиндексация
    IF @fragmentation_in_percent < 30
            SET @command = N'ALTER INDEX ' + @indexname + N' 
ON ' + @schemaname + N'.' + @objectname + N' REORGANIZE' +@partition_descr ;
    IF @fragmentation_in_percent >= 30
            SET @command = N'ALTER INDEX ' + @indexname + N' 
ON ' + @schemaname + N'.' + @objectname + N' REBUILD '+@partition_descr  + N'   WITH (SORT_IN_TEMPDB  = ON)
';

    -- Выполняем команду    
    EXEC (@command);
    PRINT N'Index: object_id=' + STR(@object_id) + ', index_id=' 
+ STR(@index_id) + ', fragmentation_in_percent=' + STR(@fragmentation_in_percent);
...
-- Таблицы обязательного индексирования
/*
  DBCC DBREINDEX ('_AccumRg15610')
  PRINT N'DBREINDEX: _AccumRg15610' ;
  DBCC DBREINDEX ('_InfoRg14225')
  PRINT N'DBREINDEX: _InfoRg14225' ;
